...
def incompleteBeta(x, p, q):
  ...
  The computation is based on formulas from Numerical Recipes, Chapter 6.4
  (W.H. Press et al, 1992). Ported from Java: http://jsci.sourceforge.net/
  """
  assert 0 <= x <= 1
  assert p > 0
  assert q > 0
...
def findRoot(value, x_low, x_high, function):
  """ Use the bisection method to find root such that f(root) == value. """
...
def tinv(p, dof):
  ...
  assert 0 <= p <= 1
  confidence = 1 - p
  return InverseStudentT(dof, (1 + confidence) / 2.0)
